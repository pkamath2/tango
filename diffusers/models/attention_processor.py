...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, attention_weights=None, **cross_attention_kwargs):
...
class AttnProcessor:
    def __call__(
        ...
        orig_encoder_hidden_states = encoder_hidden_states #PK added
        ...
        #PK added
        if orig_encoder_hidden_states is not None and orig_encoder_hidden_states.shape[1] == attention_probs.shape[-1]: #attention_weights is not None and
            # print("All satisfied in cross attention. Attention weights == ", attention_weights)
            attention_probs_rearranged = rearrange(attention_probs, "b i j -> j b i")
            wts = torch.ones_like(attention_probs_rearranged).cuda() 
            wts = wts * attention_weights[:, None, None]
            attention_probs_rearranged = attention_probs_rearranged * wts
            attention_probs_rearranged_ = rearrange(attention_probs_rearranged, "j b i -> b i j")
            attention_probs = attention_probs_rearranged_
